...
This vignette walks through a full clinical prediction modelling workflow
using triageR, from raw data to a TRIPOD+AI-aligned report. We will use the
**PIMA Indians Diabetes** dataset, a well-known real-world clinical dataset,
to demonstrate each stage of the pipeline.
...
of the kind of cleaning `tr_load_clinical()` expects to receive data
after.
...
triageR also supports `"random_forest"` and `"boost_tree"` engines via the
same interface — useful for comparing approaches without rewriting your
pipeline:

```{r}
pima_model_rf  <- tr_fit(pima_train, outcome = "diabetes", engine = "random_forest")
pima_model_xgb <- tr_fit(pima_train, outcome = "diabetes", engine = "boost_tree")
```
...
`tr_agent_review()` checks for common clinical modelling pitfalls: class
imbalance, low events-per-variable ratio, possible data leakage, and
small sample size.
...
tr_tripod_report(
  ...
  output_file = "pima_diabetes_report",
  format = "html"
)
```
...
If a Gemini API key is configured (see `?tr_recommend_method`), triageR
can also suggest an appropriate statistical or ML approach based on the
